...
var SiteApps = /** @class */ (function () {
    ...
    SiteApps.prototype.setupSiteApp = function (loader) {
        ...
        var onLoaded = function (success) {
            var e_2, _a;
            var _b;
            _this.apps[loader.id].errored = !success;
            _this.apps[loader.id].loaded = true;
            logger.info("Site app with id ".concat(loader.id, " ").concat(success ? 'loaded' : 'errored'));
            if (success && _this.bufferedInvocations.length) {
            ...
            try {
                for (var _c = __values(Object.values(_this.apps)), _d = _c.next(); !_d.done; _d = _c.next()) {
                    var app_1 = _d.value;
                    if (!app_1.loaded) {
            ...
            (_b = _this.stopBuffering) === null || _b === void 0 ? void 0 : _b.call(_this);
        };
        try {
    ...
    SiteApps.prototype.onRemoteConfig = function (response) {
        ...
        var _this = this;
        var _c, _d, _e, _f, _g;
        if ((_c = this.siteAppLoaders) === null || _c === void 0 ? void 0 : _c.length) {
            if (!this.isEnabled) {
                logger.error("PostHog site apps are disabled. Enable the \"opt_in_site_apps\" config to proceed.");
                return;
            }
            try {
                for (var _h = __values(this.siteAppLoaders), _j = _h.next(); !_j.done; _j = _h.next()) {
                    var app = _j.value;
        ...
        (_d = this.stopBuffering) === null || _d === void 0 ? void 0 : _d.call(this);
        if (!((_e = response['siteApps']) === null || _e === void 0 ? void 0 : _e.length)) {
            return;
        }
        if (!this.isEnabled) {
            logger.error("PostHog site apps are disabled. Enable the \"opt_in_site_apps\" config to proceed.");
        ...
        var _loop_1 = function (id, url) {
            ...
            (_g = (_f = assignableWindow.__PosthogExtensions__) === null || _f === void 0 ? void 0 : _f.loadSiteApp) === null || _g === void 0 ? void 0 : _g.call(_f, this_1.instance, url, function (err) {
                if (err) {
        ...
        var this_1 = this;
        try {
            for (var _k = __values(response['siteApps']), _l = _k.next(); !_l.done; _l = _k.next()) {
                var _m = _l.value, id = _m.id, url = _m.url;
                _loop_1(id, url);
            }
        }
