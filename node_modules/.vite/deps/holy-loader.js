...
var toAbsoluteURL = __name((url) => {
  return new URL(url, window.location.href).href;
}, "toAbsoluteURL");
var isSamePageAnchor = __name((currentUrl, newUrl) => {
  const current = new URL(toAbsoluteURL(currentUrl));
  const next = new URL(toAbsoluteURL(newUrl));
  return current.href.split("#")[0] === next.href.split("#")[0];
}, "isSamePageAnchor");
var isSameHost = __name((currentUrl, newUrl) => {
  const current = new URL(toAbsoluteURL(currentUrl));
  const next = new URL(toAbsoluteURL(newUrl));
  return current.hostname.replace(/^www\./, "") === next.hostname.replace(/^www\./, "");
}, "isSameHost");
var paramsAreEqual = __name((params1, params2) => Array.from(params1).every(
  ([key, value]) => params2.has(key) && params2.get(key) === value
), "paramsAreEqual");
var hasSameQueryParameters = __name((currentUrl, newUrl) => {
  const current = new URL(toAbsoluteURL(currentUrl));
...
var isSamePathname = __name((currentUrl, newUrl) => {
  const current = new URL(toAbsoluteURL(currentUrl));
  const next = newUrl instanceof URL ? newUrl : new URL(toAbsoluteURL(newUrl));
  return current.pathname === next.pathname;
}, "isSamePathname");
var clamp = __name((n, min, max) => Math.max(min, Math.min(n, max)), "clamp");
var queue = (() => {
  const pending = [];
  const next = __name(() => {
...
var repaintElement = __name((obj) => {
  void obj.offsetWidth;
  return obj;
}, "repaintElement");
var _HolyProgress = class _HolyProgress2 {
  /**
  ...
  constructor(customSettings) {
    ...
    this.toBarPercentage = __name((n) => this.settings.dir === "ltr" ? (-1 + n) * 100 : (1 - n) * 100, "toBarPercentage");
    this.start = __name(() => {
    ...
    this.complete = __name(() => this.setTo(1), "complete");
    this.calculateIncrement = __name((status) => {
      const base = 0.1;
      const scale = 5;
      return base * Math.exp(-scale * status);
    }, "calculateIncrement");
    this.incrementStatus = __name((amount) => {
      if (this.progressN === null) {
        return this.start();
      }
      if (this.progressN > 1) {
    ...
    this.createBar = __name((fromStart) => {
      var _a, _b;
...
var HolyLoader = __name(({
  ...
  React.useEffect(() => {
    ...
    const stopProgressOnHistoryUpdate = __name(() => {
      if (isHistoryPatched) {
        return;
      }
...
//# sourceMappingURL=holy-loader.js.map
